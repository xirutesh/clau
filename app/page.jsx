...
function Spin({t}){return<div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:60,gap:8,color:"#888",flexDirection:"column"}}><div style={{width:44,height:44,border:"4px solid #f0f0f0",borderTop:`4px solid ${R}`,borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>{t&&<span style={{fontSize:13,marginTop:8}}>{t}</span>}<style>{`@keyframes spin{to{transform:rotate(360deg)}}html,body{background:#f2f2f2!important;margin:0;padding:0;}img{-webkit-user-drag:none;user-select:none;-webkit-touch-callout:none;pointer-events:none;}div[style*="background"]{-webkit-touch-callout:none;user-select:none;}`}</style></div>}
