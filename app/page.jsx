...
// Info Pages
// On-site support ticket (info/legal pages). Requires a logged-in account — the
// identity comes from the session, no email needed. "Open Ticket" reveals the
// form; the user also sees their own tickets and their status.
function TicketForm({type,auth,onAuth}){
  const[open,setOpen]=useState(false);const[msg,setMsg]=useState("");const[sending,setSending]=useState(false);const[mine,setMine]=useState([]);const[loaded,setLoaded]=useState(false);
  const tok=()=>{try{return JSON.parse(localStorage.getItem("auth")||"null")?.token}catch{return null}};
  const load=useCallback(()=>{const tk=tok();if(!tk){setMine([]);setLoaded(true);return;}fetch("/api/ticket",{headers:{Authorization:`Bearer ${tk}`}}).then(r=>r.ok?r.json():[]).then(d=>{setMine(Array.isArray(d)?d.filter(t=>t.type===type):[]);setLoaded(true);}).catch(()=>setLoaded(true));},[type]);
  useEffect(()=>{load()},[load,auth]);
  const submit=async()=>{if(msg.trim().length<10){alert("Please describe your request (at least 10 characters).");return;}setSending(true);try{const tk=tok();const r=await fetch("/api/ticket",{method:"POST",headers:{"Content-Type":"application/json",...(tk?{Authorization:`Bearer ${tk}`}:{})},body:JSON.stringify({type,message:msg.trim()})});if(r.ok){setMsg("");setOpen(false);load();}else{const d=await r.json().catch(()=>({}));alert(d.error||"Could not send. Please try again.");}}catch{alert("Could not send. Please try again.");}setSending(false);};
  ...
  const btn={padding:"12px 26px",borderRadius:10,border:"none",background:G,color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer"};
  if(!auth)return<div style={{textAlign:"center"}}><p style={{color:"#444",fontSize:14,marginBottom:12}}>Log in to open a ticket and track its status.</p><button onClick={onAuth} style={btn}>Log in</button></div>;
  return<div style={{textAlign:"center"}}>
    {!open?<button onClick={()=>setOpen(true)} style={btn}>Open Ticket</button>
    :<div><textarea placeholder="Describe your request..." value={msg} onChange={e=>setMsg(e.target.value)} style={{...inp,minHeight:100,resize:"vertical",textAlign:"left"}}/><div style={{display:"flex",gap:8}}><button onClick={submit} disabled={sending} style={{flex:1,padding:12,borderRadius:8,border:"none",background:G,color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",opacity:sending?0.7:1}}>{sending?"Sending...":"Submit Ticket"}</button><button onClick={()=>{setOpen(false);setMsg("")}} style={{padding:"12px 16px",borderRadius:8,border:"1px solid #ccc",background:"#fff",color:"#555",fontWeight:700,cursor:"pointer"}}>Cancel</button></div></div>}
    {loaded&&mine.length>0&&<div style={{marginTop:16,textAlign:"left"}}><div style={{fontWeight:700,fontSize:13,color:"#333",marginBottom:8}}>Your tickets</div>{mine.map(t=><div key={t.id} style={{background:"#fff",border:"1px solid #eee",borderRadius:8,padding:"10px 12px",marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:11,padding:"2px 8px",borderRadius:4,fontWeight:700,background:t.status==="closed"?"#E8F5E9":"#FFF3E0",color:t.status==="closed"?"#2E7D32":"#E65100"}}>{(t.status||"open").toUpperCase()}</span><span style={{fontSize:11,color:"#999"}}>{t.created_at?new Date(t.created_at).toLocaleDateString():""}</span></div><div style={{fontSize:13,color:"#444",whiteSpace:"pre-wrap"}}>{t.message}</div></div>)}</div>}
  </div>;
}

function InfoP({page,config,auth,onAuth}){
  const TG="https://t.me/uflashbrazilofficial";
  if(page==="p053")return<div style={{background:"#fff",borderRadius:12,padding:20,margin:16,textAlign:"center"}}><div style={{fontWeight:800,fontSize:20,marginBottom:8,color:"#1a1a1a"}}>Contact Us on Telegram</div><p style={{color:"#444",fontSize:14,lineHeight:1.6,marginBottom:20}}>Have a question or need help? Message us directly on Telegram — we usually reply fast.</p><a href={TG} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",padding:"14px 32px",borderRadius:10,background:"#229ED9",color:"#fff",fontWeight:700,fontSize:16,textDecoration:"none"}}>Contact us</a></div>;
  if(page==="p041")return<div style={{margin:"0 16px"}}><div style={{borderTop:"1px solid #ddd"}}/><div style={{padding:"12px 0",borderBottom:"1px solid #ddd"}}><span style={{fontWeight:700,fontSize:16}}>18 USC 2257</span></div><div style={{padding:"16px 0",fontSize:15,color:"#444",lineHeight:1.7}}><p style={{marginBottom:14,textTransform:"uppercase",fontWeight:600}}>ALL PICTURES ARE PRESENTED BY THIRD PARTIES.</p><p style={{marginBottom:14,textTransform:"uppercase",fontWeight:600}}>ALL MODELS ARE 18 YEARS OF AGE OR OLDER IN COMPLIANCE WITH 18 USC 2257</p><p style={{fontWeight:600,fontStyle:"italic"}}>All visitors of this website are required to be over 18 years old (over 21 in some locations).</p><div style={{background:"#FFF3E0",borderRadius:10,padding:14,marginTop:16}}><div style={{fontWeight:700,fontSize:14,color:"#333",marginBottom:10}}>For any legal question, open a ticket:</div><TicketForm type="legal" auth={auth} onAuth={onAuth}/></div></div></div>;
  if(page==="p072")return<div style={{margin:"0 16px"}}><div style={{borderTop:"1px solid #ddd"}}/><div style={{padding:"12px 0",borderBottom:"1px solid #ddd"}}><span style={{fontWeight:700,fontSize:16}}>Content Removal</span></div><div style={{padding:"16px 0",fontSize:14,color:"#444",lineHeight:1.8}}><p style={{marginBottom:12}}>If you appear in any content and wish to have it removed, submit a removal request.</p><div style={{background:"#E8F5E9",borderRadius:10,padding:14}}><div style={{fontWeight:700,fontSize:14,color:"#333",marginBottom:10}}>To request removal, open a ticket:</div><TicketForm type="removal" auth={auth} onAuth={onAuth}/></div></div></div>;
  return null;
}
...
function Admin({auth,channels,setChannels,config,setConfig,onClose,reload,onLogout}){
  ...
  // When you start editing, scroll up to the form so you don't have to scroll by hand
  // after each product.
  ...
  const edFiltered=channels.filter(c=>catF==="all"?true:catF==="__nophoto__"?!c.image_url:catF==="__nocount__"?!(c.video_count>0):(c.category||"")===catF).filter(c=>{const qs=chSearch.split("\n").map(l=>l.trim().toLowerCase()).filter(Boolean);return !qs.length||qs.some(q=>(c.name||"").toLowerCase().includes(q));});
  ...
  return<div style={{position:"fixed",inset:0,background:"#f5f5f5",zIndex:2000,overflowY:"auto"}}>
    ...
    {tab==="tickets"&&<div style={{padding:16}}>
      ...
      {tickets.map(t=><div key={t.id} style={{background:"#fff",borderRadius:10,padding:14,marginBottom:10,border:"1px solid #eee",opacity:t.status==="closed"?0.6:1}}>
        ...
        {t.username&&<div style={{fontSize:12.5,color:"#1565C0",marginBottom:4,wordBreak:"break-all"}}>User: {t.username}</div>}
    ...
    {tab==="edit"&&<div style={{padding:16}}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",marginBottom:10,paddingBottom:2}}>{["all","__nophoto__","__nocount__",...cats].map(c=>{const special=c==="__nophoto__"||c==="__nocount__";const n=c==="all"?channels.length:c==="__nophoto__"?channels.filter(x=>!x.image_url).length:c==="__nocount__"?channels.filter(x=>!(x.video_count>0)).length:channels.filter(x=>(x.category||"")===c).length;const lbl=c==="all"?"All":c==="__nophoto__"?"🚫 No photo":c==="__nocount__"?"🔢 No count":c;const active=catF===c;return<button key={c} onClick={()=>setCatF(c)} style={{flexShrink:0,padding:"6px 12px",borderRadius:20,border:active?`2px solid ${special?"#c0392b":G}`:"1px solid #ddd",background:active?(special?"#c0392b":G):"#fff",color:active?"#fff":special?"#c0392b":"#555",fontWeight:700,fontSize:12,cursor:"pointer"}}>{lbl} ({n})</button>})}</div>
